...
- **Never reintroduce the phone number.** It was intentionally removed for privacy. Contact = email + LinkedIn (`linkedin.com/in/h1sort`) + GitHub (`github.com/h1sort`) + X (`x.com/h1sort`) + CV PDF + location only.
...
- `src/pages/polls.astro` — private poll control room and public voting surface. Admin auth, poll CRUD, status changes, one-vote-per-browser enforcement, and closed-only public results are handled by `/api/polls/*` in `worker/index.ts`; `/p/<code>` redirects to the voter view. Poll data shares the `h1sort-chat` D1 binding and is defined by `migrations/0002_poll_tables.sql` and `migrations/0003_poll_auth_rate_limits.sql`. QR codes are generated locally in the browser.
